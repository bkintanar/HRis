...
new Vue({
    ...
    filters: {
        timeFormat: function timeFormat(utc) {
            if (!utc) return '--:--';
            return this.utcToLocal(utc, 'hh:mm A');
        },
        dateFormat: function dateFormat(utc) {
            return this.utcToLocal(utc, 'MMM D, YYYY');
        },
        decimalPlace: function decimalPlace(num) {
    ...
    methods: {
        ...
        initTimelogId: function initTimelogId() {
            this.timelog_id = this.$$.out.getAttribute('data-id');
        },
        utcToLocal: function utcToLocal(utc, format) {
            var local = moment.utc(utc).toDate();
            return moment(local).format(format);
        },
        onApplyDateRange: function onApplyDateRange() {
            var self = this;
        ...
        fetchTimelogs: function fetchTimelogs() {
        ...
        deleteTimelog: function deleteTimelog(timelog) {
            var self = this;
        ...
        editTimelog: function editTimelog(timelog) {
            var self = this,
                data = {
...
},{"./batcher":17,"./config":23,"./observer/dep":57,"./parsers/expression":61,"./util":72,"_process":2}]},{},[1]);
